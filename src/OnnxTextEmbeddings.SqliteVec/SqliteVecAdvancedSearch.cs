...
public sealed record SqliteVecSearchPlan
{
    public SqliteVecCandidateQuery? Semantic { get; init; }
    public SqliteFts5Query? Lexical { get; init; }
}

/// <summary>Executes composable semantic/FTS5 SearchQuery plans and fuses native SQLite rankings with core RRF.</summary>
public sealed class SqliteVecAdvancedSearch(
    ITextEmbeddingService embeddingService,
    SqliteVecSemanticSearch semanticSearch,
    SqliteFts5LexicalSearch lexicalSearch)
{
    public async Task<IReadOnlyList<SearchResult<TKey>>> SearchAsync<TKey>(
        SqliteConnection connection,
        SearchQuery query,
        SqliteVecSearchPlan plan,
        QueryEmbedding? semanticQuery = null,
        Func<TKey, IReadOnlyDictionary<string, object?>>? postFilterValues = null,
        Action<SqliteCommand>? configureProviderParameters = null,
        ...
        where TKey : notnull
    {
        ...
        foreach (var stage in query.Retrievals)
        {
            ...
            {
                var configured = plan.Semantic ?? throw new InvalidOperationException("The SearchQuery contains a semantic stage but the SqliteVecSearchPlan has no Semantic mapping.");
            ...
            {
                var configured = plan.Lexical ?? throw new InvalidOperationException("The SearchQuery contains a lexical stage but the SqliteVecSearchPlan has no Lexical mapping.");
